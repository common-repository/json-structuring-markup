...
/**
 * Schema.org NewsArticle Settings Page
 */
function json_structuring_markup_newsarticle_settings_page() {
  ...
  if (  isset($_POST['submit']) ){
    ...
    update_option('json_structuring_markup_newsarticle', serialize( $schema ) );
  }
  $schema_newsarticle_settings = unserialize( get_option('json_structuring_markup_newsarticle') );
  if( isset($schema_newsarticle_settings) ){
    $default_image    =     $schema_newsarticle_settings['default_image'];
    $org_name         =     $schema_newsarticle_settings['org_name'];
    $org_logo         =     $schema_newsarticle_settings['org_logo'];
    $activated        =     esc_attr( $schema_newsarticle_settings['activated'] );
  ...
  echo '<div class="wrap">';
  echo '<h1>NewsArticle SCHEMA Markup SETTINGS</h1>';
  echo '<div>Provide the fields to make the validated schema for NewsArticle.</div>';
  echo '<form enctype="multipart/form-data" action="" method="POST" id="newsarticle-schema">';
?>
  ...
  <table class="schema-admin-table">
    <tbody>
      <tr>
        <td><input type="checkbox" name="activated" value="on" <?php echo $schema_activated_checked; ?> /><strong>Activate SCHEMA.ORG NewsArticle</strong></td>
      </tr>
    </tbody>
  </table>

  <p>Setting Knowledge : <a href="https://developers.google.com/structured-data/rich-snippets/articles" title="NewsArticle Schema" target="_blank">https://developers.google.com/structured-data/rich-snippets/articles</a></p> 
<?php
  submit_button();
  echo '</form>';
  echo '</div>';
...
/**
 * Print JSON-LD NewsArticle schema markup
 */
function json_structuring_markup_display_newsarticle(){
  $schema_newsarticle_settings = unserialize( get_option('json_structuring_markup_newsarticle') );
  $activated = '';
  if( isset($schema_newsarticle_settings) ) {
    $default_image    =     esc_attr( $schema_newsarticle_settings['default_image'] );
    $org_name         =     esc_attr( $schema_newsarticle_settings['org_name'] );
    $org_logo         =     esc_attr( $schema_newsarticle_settings['org_logo'] );
    $activated        =     esc_attr( $schema_newsarticle_settings['activated'] );
  }
  if ( $activated == 'on' ) {
    global $post;
